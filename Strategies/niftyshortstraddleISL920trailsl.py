...
class NiftyShortStraddleISL920(BaseStrategy):
    ...
    def get_trailing_sl(self, trade):
        ...
        if trade.entry == 0:
            return 0
        ltp = TradeManager.get_ltp_trade(trade).last_traded_price
        if ltp == 0:
            return 0
        
        trail_sl = 0
        profit_points = trade.entry - ltp
        if profit_points >= 5:
            factor = profit_points//5
            trail_sl = round_to_nse_price(trade.initial_stoploss - (factor*5))
